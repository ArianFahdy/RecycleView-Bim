...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_signin.setOnClickListener {
            ...
            for (i in 0 until Utils.movies.size){
                if (movieManager.getMovies()[i].name != Utils.movies[i].name) {
                    movieManager.insert(
                        Utils.movies[i].name,
                        Utils.movies[i].popularity.toString(),
                        Utils.movies[i].imagePath
                    )
                }
            }


            Toast.makeText(this, "username ${edt_username.text} dan password anda ${edt_password.text}", Toast.LENGTH_SHORT).show()
